...
- Creat a new repository called SMPI-benchmarks that contain un org mode file. Through this latter one can launch the download of each benchmark and 
make the necessary changes that allow to run it with smpi.
- The org mode file contain also an organized table for all the candidate benchmark codes with a brief description.