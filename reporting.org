...
- Congestion factor, SST/macro has three network models : the analytical model transfers, the packet-level model PISCES, and the flow-level model.
...
- They modified SST/macro to create a gloval v ariable and attach the shared memory to this global variable. (p11)
