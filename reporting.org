...
TODO
  - smpicheckcompat: script à ajouter à SimGrid à coté de smpirun
    - Version0: qui marche sur ta machine (seulement)
    - Version1: qui utilise CMAKE_HOME_DIRECTORY pour retrouver la
      racine des sources de simgrid
    - Version2: qui extrait la liste des fonctions utilisables lors de
      la compilation (de la création du script) pour marcher même si
      les sources de simgrid sont supprimées du disque
  - Automatiser l'exécution des tests, afin de permettre le debug
    - Ajouter un job dans jenkins "smpi-benchmarks" qui relance tout
      chaque nuit
    - Pour l'instant, on veut un truc simple, où jenkins voit les
      tests comme un seul grand blurb, sans le détail des tests
    - Un jour peut-être, on voudra que chaque bench soit vu comme un
      test unitaire de SMPI